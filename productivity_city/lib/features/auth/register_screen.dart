...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (email.isEmpty || username.isEmpty || password.isEmpty) {
      _showMessage('Заполни все поля перед регистрацией.');
      return;
    }
    ...
    if (username.length < 3) {
      _showMessage('Имя пользователя должно содержать минимум 3 символа.');
      return;
    }
    if (password.length < 8) {
      _showMessage('Пароль должен содержать минимум 8 символов.');
      return;
    }
    ...
    try {
      ...
      _showMessage('Не удалось создать аккаунт. Попробуй еще раз.');
    } finally {
      if (mounted) {
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Center(
                    child: Text(
                      'Начните строить город своей мечты.',
                      textAlign: TextAlign.center,
                  ...
                  const _AuthLabel(label: 'Имя пользователя'),
                  const SizedBox(height: 10),
                  _AuthInput(
