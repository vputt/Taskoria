...
Taskoria построена как клиент-серверное приложение: Flutter-клиент отправляет запросы к FastAPI backend, backend проверяет авторизацию, применяет бизнес-логику и сохраняет данные в PostgreSQL.

![Общая архитектура Taskoria](readme_assets/architecture.png)

Основной пользовательский путь строится вокруг цикла: создание задачи, декомпозиция на подзадачи, выполнение, получение награды и визуальное обновление города.

![Пользовательский путь Taskoria](readme_assets/user_flow.png)
